...
class NotificationPreferenceAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        """Prevent adding NotificationPreference directly from admin."""
        return False
...
class ResumeAdmin(admin.ModelAdmin):
    list_display = ('id', 'title', 'full_name', 'user', 'email', 'phone', 'updated_at')
    search_fields = ('title', 'full_name', 'email', 'skills')
    list_filter = ('template_key', 'updated_at')
    ordering = ('-updated_at',)
    fieldsets = (
        ('Basic Information', {
            'fields': ('user', 'title', 'full_name', 'date_of_birth', 'headline', 'summary', 'photo', 'template_key')
        }),
        ('Contact Details', {
            'fields': ('email', 'phone', 'location', 'linkedin', 'website', 'github')
        }),
        ('Resume Content', {
            'fields': ('experiences', 'educations', 'skills', 'languages', 'awards')
        }),
        ('Extra Information', {
            'fields': ('extra_info_1', 'extra_info_2', 'extra_info_3')
        }),
    )
